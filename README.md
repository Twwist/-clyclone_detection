...
Также даны веса (**weights.pt**) которые вычисляются с помощью модели YOLO, и позже **solution.py** использует эти веса для предсказания результата. 

**ОБЯЗАТЕЛЬНО**: создать папку photos (в той же директории что и **solution.py**), где будут храниться все картинки. А также скачать **weight.pt** и сохранить также в той же папке что и **solution.py**.
